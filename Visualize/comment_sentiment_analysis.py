...
def login(path):
    ...
    webbrowser.get('Safari').open("https://dyf-2316.github.io/LDA_Results/" + path)
...
section = st.sidebar.radio("请选择需要展示的项目模块：", ('0. 项目介绍与规划', '1. 数据采集与抽取', '2. 数据预处理与探索', '3. 自训练情感分析模型', '4. 评论分词与改进',
                                             '5. 词云与语义网络构建', '6. LDA主题模型构建', '7. 交互诊断与反馈'))

if section in ['5. 词云与语义网络构建', '6. LDA主题模型构建', '7. 交互诊断与反馈']:
    st.sidebar.markdown('# 评论标签')
    tag = st.sidebar.selectbox("  请选择分析的评论标签：", ('总体评论', '外形外观', '恒温效果', '噪音大小', '出水速度', '安装服务', '耗能情况', '加热速度', '洗浴时间', '其他特色'))

if section in ['6. LDA主题模型构建']:
    st.sidebar.markdown('# LDA主题数')
    topic_number = st.sidebar.slider('  请选择需要训练的LDA主题数:', 3, 9, 5)


if section == '0. 项目介绍与规划':
    st.markdown('## 0. 项目介绍与规划')

    st.markdown('### 0.1 开发人员清单与分工')
    st.markdown('- 丁一凡：1、 数据爬取与数据抽取')
    st.markdown('&emsp;'*20 + '2、数据预处理以及标签评论的处理')
    st.markdown('&emsp;'*20 + '3、撰写会议记录')
    st.markdown('&emsp;'*20 + '4、Streamlit可视化呈现最终结果')
    st.markdown('&emsp;'*20 + '5、将项目搭建在Heroku云服务器上')
    st.markdown('- 鲁含章：1、对整体评论数据与分标签评论数据进行LDA主题分析模型建模及可视化')
    st.markdown('&emsp;'*20 + '2、使用ROST对分标签评论数据进行语义网络分析')
    st.markdown('&emsp;'*20 + '3、需求文档撰写及更新等')
    st.markdown('- 马生鸿：1、对整体评论数据和分标签评论数据进行词云的绘制')
    st.markdown('&emsp;'*20 + '2、撰写相关文档')
    st.markdown('- 李毅: 1、使用RoBERTa - wwm - ext预训练网络实现文本情感预测，对比ROST预测正确率提升25 %，并对不同的标签的评论进行情感分析')
    st.markdown('&emsp;'*15 + '2、完成模型部分可视化')
    st.markdown('&emsp;'*15 + '3、将项目搭建在Heroku云服务器上')
    st.markdown('&emsp;'*15 + '4、完成项目展示报告，项目立项书等')

    st.markdown('### 0.2 项目开发环境清单')
    environment_list = pd.DataFrame({'名称': ['Windows10 & MacBook Pro', 'pycharm + google colab', 'python3.7', 'MongoDB', 'GitHub', 'Heroku'],
    ...
    st.markdown('### 0.4 项目实用技术')
    tech_list = pd.DataFrame({'模块': ['数据爬取', '数据存储', '数据预处理', '隐含信息挖掘', '情感分析', '评论分词', '词云与语义网络', 'LDA主题模型', '模型评估与诊断', '可视化'],
